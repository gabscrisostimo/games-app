...
**Status: camada genérica + PoC + adaptador do Quiplash real (proof #1) IMPLEMENTADOS e testados (código).** Falta só o teste real nos celulares — **bloqueado** (ver "Limitações"). Branch `feat/multiplayer` em `.claude/worktrees/feat+multiplayer`, **rebaseado em `origin/main`** (que já tem o Quiplash mergeado — não é mais `e8f6399`). Spec: `docs/superpowers/specs/2026-06-13-multiplayer-netcode-design.md`. Plano: `docs/superpowers/plans/2026-06-13-multiplayer-netcode.md`.
...
- **Generalização (regra do três):** demo (1º uso) → **Quiplash real via adaptador (2º uso — proof #1 ✅, o seam segurou)** → **Spyfall** (papel secreto estático vs projeção por-fase) é o gatilho do próximo ciclo, fora deste escopo. **Achado do proof #1:** o contrato segurou estado/projeção/rng, mas o reducer pass-and-play do Quiplash embute um **modelo de turno sequencial** (`voteCursor`) que o adaptador teve que **traduzir pra votação concorrente por ator**. Sinal pra avaliar (depois do Spyfall) se a forma certa é "engines expõem reducer endereçável por ator".
...
- **Teste de celular BLOQUEADO (2026-06-13):** `npx partykit deploy` falha — a zona compartilhada `*.partykit.dev` bateu o teto de 10k domínios custom da Cloudflare (lado da plataforma, não da conta). Some-se: o app roda em **WSL2** (atrás de NAT), então `vite dev --host` não é alcançável direto pelos celulares na LAN. Opções p/ destravar quando for a hora: túnel público (cloudflared) expondo app+room server, ou self-host do room server na conta Cloudflare própria (domínio próprio). Decisão atual: **adiado** — proof #1 (código) tem prioridade e não depende de rede.
