...
**Status: camada genérica + PoC jogável IMPLEMENTADAS e testadas (código).** Falta só o teste real nos 2 celulares (deploy PartyKit — depende do Gabs rodar `npx partykit login`, interativo). Branch `feat/multiplayer` em `.claude/worktrees/feat+multiplayer` (base = `main` `e8f6399`). Spec: `docs/superpowers/specs/2026-06-13-multiplayer-netcode-design.md`. Plano: `docs/superpowers/plans/2026-06-13-multiplayer-netcode.md`.
...
- **O que a PoC cobre:** lobby genérico (criar→código 4 letras→entrar c/ apelido) + presença + reconexão (por `playerId` em localStorage) + sincronização com **projeção secreta por-jogador** + timer autoritativo. Validada por um **demo promptvote descartável** (`src/net/__demo__/`). Verificação: **129 testes** verdes, `tsc -b` limpo, servidor PartyKit boota, e um **smoke end-to-end com 3 clientes `PartySocket` reais** (join→start→answer→vote→reveal) passou incluindo secrecy no fio e voto anônimo.
...
Cada jogo que quiser multi-device implementa esta interface (de `src/net/contract.ts`). A autoridade roda `createInitial`/`reducer`/`onTimeout` (dona de `now`/`rng`); o cliente só renderiza `project(state, playerId)`. **A Engine 3 (promptvote/Quiplash) NÃO precisa saber de rede:** quando o `logic.ts` do Quiplash existir, o Chat Backend escreve um adaptador fino em `src/net/adapters/quiplash.ts` que importa o reducer/projeção da engine — **sem editar `src/games/promptvote/**`**.
...
- **Generalização (regra do três):** demo (1º uso) → Quiplash real via adaptador (2º uso, prova o seam) → **Spyfall** (papel secreto estático vs projeção por-fase) é o gatilho do próximo ciclo, fora deste escopo.
- **Limitações conhecidas de PoC (não-bloqueantes):** host não transfere ao cair (reconexão por playerId estável traz de volta); estado da sala em memória (sem persistência durável); deploy do app pro teste = `vite dev --host` no wifi local + PartyKit deployado.
