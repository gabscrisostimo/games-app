...
**Status: camada genérica + PoC + adaptador do Quiplash real (proof #1) IMPLEMENTADOS e VALIDADOS EM CELULAR REAL.** O demo (`net-demo.html`) foi jogado em **Android + iOS + 3º device** (2026-06-13): lobby/presença, projeção secreta, voto anônimo, placar e **reconexão no iOS** (lock de tela ~30s → volta) — tudo OK. Branch `feat/multiplayer` em `.claude/worktrees/feat+multiplayer`, **rebaseado em `origin/main`** (que já tem o Quiplash mergeado — não é mais `e8f6399`). Spec: `docs/superpowers/specs/2026-06-13-multiplayer-netcode-design.md`. Plano: `docs/superpowers/plans/2026-06-13-multiplayer-netcode.md`.
